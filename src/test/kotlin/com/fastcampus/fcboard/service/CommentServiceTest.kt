...
import com.fastcampus.fcboard.exception.CommentNotDeleteException
...
class CommentServiceTest(
    ...
    given("댓글 수정시") {
        ...
        val saved = commentRepository.save(
            Comment(content = "댓글 내용", createdBy = "댓글 생성자", post = post)
        )
        When("인풋이 정상적으로 들어오면") {
            ...
            then("정상 수정됨을 확인한다.") {
                ...
                val comment = commentRepository.findByIdOrNull(updatedId)
                comment shouldNotBe null
    ...
    given("댓글 삭제시") {
        ...
        val saved2 = commentRepository.save(
            Comment(content = "댓글 내용2", createdBy = "댓글 생성자2", post = post)
        )
        When("인풋이 정상적으로 들어오면") {
